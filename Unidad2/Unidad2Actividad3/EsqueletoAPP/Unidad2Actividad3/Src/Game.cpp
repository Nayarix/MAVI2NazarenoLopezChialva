...
void Game::InitPhysics() {
    ...
    pelotaBody = Box2DHelper::CreateCircularDynamicBody(phyWorld, 15.0f, 1.0f, 0.05f, 1.0f);
    pelotaBody->SetTransform(b2Vec2(wnd->getSize().x / 2.0f, wnd->getSize().y / 2.0f), 0.0f);
    pelotaBody->ApplyLinearImpulse(b2Vec2(5000.0f, -5000.0f), pelotaBody->GetWorldCenter(), true);
    ...
    obstaculos.push_back(Box2DHelper::CreateRectangularStaticBody(phyWorld, 50.0f, 50.0f));
    obstaculos.back()->SetTransform(b2Vec2(150.0f, 200.0f), 0.0f);
    obstaculos.push_back(Box2DHelper::CreateRectangularStaticBody(phyWorld, 80.0f, 30.0f));
    obstaculos.back()->SetTransform(b2Vec2(400.0f, 300.0f), 0.785f);
    obstaculos.push_back(Box2DHelper::CreateTriangularStaticBody(phyWorld, b2Vec2(0.0f, 0.0f), 60.0f));
    obstaculos.back()->SetTransform(b2Vec2(600.0f, 150.0f), 0.0f);
}
...
void Game::DrawGame() {
    ...
    for (auto& obs : obstaculos) {
        b2PolygonShape* shape = (b2PolygonShape*)obs->GetFixtureList()->GetShape();
        sf::ConvexShape convex;
        convex.setPointCount(shape->m_count);
        for (int i = 0; i < shape->m_count; ++i)
            convex.setPoint(i, sf::Vector2f(shape->m_vertices[i].x, shape->m_vertices[i].y));
        convex.setPosition(obs->GetPosition().x, obs->GetPosition().y);
        convex.setRotation(obs->GetAngle() * 180.0f / b2_pi);
        convex.setFillColor(sf::Color(200, 50, 50));
        wnd->draw(convex);
    }
}
...
void Game::DoEvents() {
    ...
    while (wnd->pollEvent(evt)) {
        ...
        if (evt.type == sf::Event::KeyPressed && evt.key.code == sf::Keyboard::R) {
            pelotaBody->SetTransform(b2Vec2(wnd->getSize().x / 2.0f, wnd->getSize().y / 2.0f), 0.0f);
            pelotaBody->SetLinearVelocity(b2Vec2(5000.0f, -5000.0f));
        }
    }
}
